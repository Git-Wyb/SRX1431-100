...
#include "spi.h"
	...
	typedef struct										//Packet Preamble
	    ...
	    unsigned short TX_PREAM_SIZE;				//tx preamble length					//## note: for arm mcu unsigned short int is 16bits
		}PREAMBLE_CFG;
	...
	typedef struct
		...
		union
			...
			unsigned short CODING_CFG_REG;			// CTL_REG_82, 0x52 & CTL_REG_93, 0x5D
			}CODING_FORMAT_CFG_u;
		unsigned short WHITEN_SEED;					// CTL_REG_83, 0x53 & CTL_REG_82[6]
		unsigned short FEC_PAD_CODE;				// CTL_REG_94, 0x5E & CTL_REG_93[6:2], 0x5D
		}CODING_FORMAT_CFG;
	...
	typedef struct
		...
		union
			...
			struct
				...
				unsigned char 	ADDR_FIELD_EN:		1;	//0: disable;   			1: enable address field
		...
		unsigned short TX_PKT_NUM;					//CTL_REG_85, 0x55[7:0] & CTL_REG_86, 0x56[15:8]
		unsigned short SEQNUM_TX_IN;				//CTL_REG_88/87, 0x58[15:8], 0x57[7:0]
		unsigned short SEQNUM_TX_CURRENT_OUT;		//CTL_REG_39/38, 0x27[15:8]/0x26[7:0]
		...
		unsigned short PAYLOAD_LENGTH;				//CTL_REG_62/61, 0x3E[15:8], 0x3D[7:0]
		}FRAME_CFG;
	...
	typedef struct
		...
		union
			...
			struct
				{
				unsigned char	RESV_5:             5;
				unsigned char   FREQ_SW_STATE:      1;	//0:switch to RFS/TFS, and then to the next channel;   1:switch to RDY, and then to the next channel;
				unsigned char 	RX_HOP_PERSIST:     1;	//0:depend on TX/RX_EXIT_STATE, 					   1:go on next cycle from 0 label
		...
		unsigned short SLEEP_TIMER_M;				//CTL_REG_100[7:5]|CLT_REG_99, 11bits, 0x64,0x63
		unsigned char	   SLEEP_TIMER_R;				//CTL_REG_100[4:0], 0x64
		unsigned short RX_TIMER_T1_M;				//CTL_REG_102[7:5]|CLT_REG_101, 11bits, 0x66, 0x65
		unsigned char	   RX_TIMER_T1_R;				//CTL_REG_102[4:0], 0x66
		unsigned short RX_TIMER_T2_M;				//CTL_REG_104[7:5]|CLT_REG_103, 11bits, 0x68, 0x67
		unsigned char	   RX_TIMER_T2_R;				//CTL_REG_104[4:0], 0x68
		unsigned short RX_TIMER_CSMA_M;				//CTL_REG_108[7:5]|CLT_REG_107, 11bits, 0x6C,0x6B
		...
		unsigned short SLEEP_TIMER_CSMA_M;			//CTL_REG_119[7:5]|CTL_REG_118, 11bits
	...
	typedef struct
		...
		unsigned long 	  CDR_BR_TH;					//RX_CDR_REG_03<18:16> & RX_CDR_REG_02<15:8> & RX_CDR_REG_01<7:0>, page1(0x47,0x45,0x46)
		}CDR_TRACING_CFG;
